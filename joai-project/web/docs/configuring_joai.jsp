...
<p>&nbsp;</p>
<hr>
<h3><a name="accessControl"></a>Enable access control in Tomcat</h3>
...
password protection for the administrative pages, do the following (this assumes use of at least Tomcat v 5.x):</p>
<p>1. Uncomment the 'security-constraint' and 'login-config' elements found 
in the oai "<span class="code">WEB-INF/web.xml</span>" file. Assuming a default installation, this
...
<p>2. Copy the following Context and Realm elements into the <span class="code">&lt;Host&gt;</span>
element of the 'server.xml' configuration file found in the Tomcat 5.x 'conf'
directory (the configuration below assumes the OAI context and directory name is 'oai'):</p>
<blockquote>
  <p class="code">&lt;Realm className="org.apache.catalina.realm.MemoryRealm" pathname="webapps/oai/WEB-INF/users.xml"/&gt;</p>
  <p class="code">&lt;Context path="/oai" docBase="oai" debug="0" reloadable="true"/&gt;</p>
</blockquote>
<p>3. Edit the file '<span class="code">WEB-INF/users.xml</span>' to define the user names and passwords 
for those you wish to grant access to the OAI administration pages. An 
example user and instructions are provided in that file.</p>
<p>4. Start or restart Tomcat. </p>
<p>Note that this type of authentication does not provide encryption for 
user names and passwords sent over the Internet.</p>
<p><br>
</p>
<hr>
<h3><a name="utf8"></a>Configure Tomcat's URI encoding for UTF-8</h3>
