...
<ListSets>
	<x:forEach select="$listSetsConfXml/ListSets/set" var="set">
		<set> <c:set var="setSpec"><x:out select="$set/setSpec"/></c:set>
			<c:if test="${initParam.showNumRecordsInOAIListSets != 'false'}">
			<!-- This set contains <c:set var="numRecordsInSet"><bean:write name="repositoryManager" property="numRecordsInSet(${setSpec})" filter="false"/></c:set>${numRecordsInSet} record${numRecordsInSet == 1 ? '' : 's'} -->
			</c:if>
			<setSpec>${setSpec}</setSpec>	
			<setName><x:out select="$set/setName"/></setName>
			<setDescription>
				<oai_dc:dc xmlns:oai_dc="http://www.openarchives.org/OAI/2.0/oai_dc/" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.openarchives.org/OAI/2.0/oai_dc/ http://www.openarchives.org/OAI/2.0/oai_dc.xsd">
					<dc:title><x:out select="$set/setName"/></dc:title>
