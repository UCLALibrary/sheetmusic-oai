<?xml version="1.0"?>
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:d="http://www.dlese.org/" exclude-result-prefixes="xsi d" version="1.0">
	...
	<!--22. For DWEL, SERC-EET, SERC-STARTINGPOINT and SERC-NAGT all relations with DLESE are unintentional and therefore must be corrected-->
	...
	<!--	variable for catalog record number metametadata.catalogentry.entry.langstring-->
	<!--	<xsl:variable name="catalognumber">
		<xsl:value-of select="d:record/d:metametadata/d:catalogentry/d:entry/d:langstring"/>
	</xsl:variable> -->
	<xsl:template match="*|/">
	...
	<xsl:template match="d:record">
		...
		<xsl:element name="general">
			...
			<xsl:element name="title">
				<xsl:apply-templates select="d:general/d:title"/>
				<!--see LANGSTRING template-->
			</xsl:element>
			<xsl:value-of select="$newline"/>
			<!--general description-->
			<xsl:apply-templates select="d:general/d:description"/>
			<!--see DESCRIPTION template-->
			<xsl:value-of select="$newline"/>
			<!--general language-->
			<xsl:if test="string-length(d:general/d:language)>0">
				<xsl:apply-templates select="d:general/d:language"/>
				<!--see LANUAGE template-->
				<xsl:value-of select="$newline"/>
			</xsl:if>
			<!--begin subjects-->
			...
			<xsl:element name="subjects">
				<xsl:value-of select="$newline"/>
				<xsl:apply-templates select="d:general/d:extension/d:topic"/>
				<!--see TOPIC template-->
			</xsl:element>
			<xsl:value-of select="$newline"/>
			<!--end subjects-->
			...
			<xsl:if test="string-length($allkeywords)>0">
				<xsl:element name="keywords">
					<xsl:value-of select="$newline"/>
					<xsl:apply-templates select="d:general/d:keywords"/>
					<!-- see KEYWORDS template-->
				</xsl:element>
				<xsl:value-of select="$newline"/>
			</xsl:if>
			<!--end keywords above-->
			...
			<xsl:if test="string-length($allcatentry)>0">
				<xsl:element name="catalogEntries">
					<xsl:value-of select="$newline"/>
					<xsl:apply-templates select="d:general/d:catalogentry"/>
					<!--see CATALOGENTRY template-->
				</xsl:element>
				<xsl:value-of select="$newline"/>
			</xsl:if>
		...
		<xsl:element name="lifecycle">
			...
			<xsl:if test="string-length(d:lifecycle/d:version/d:langstring)>0">
				<xsl:element name="version">
					<xsl:apply-templates select="d:lifecycle/d:version"/>
					<!--see LANGSTRING template-->
				</xsl:element>
				<xsl:value-of select="$newline"/>
			</xsl:if>
			...
			<xsl:if test="string-length($allroles)>0">
				<xsl:element name="contributors">
					<xsl:value-of select="$newline"/>
					<xsl:apply-templates select="d:lifecycle/d:contribute"/>
					<!--see CONTRIBUTE template-->
				</xsl:element>
				<xsl:value-of select="$newline"/>
			</xsl:if>
		...
		<xsl:element name="metaMetadata">
			...
			<xsl:if test="string-length($allmcatentry)>0">
				<xsl:element name="catalogEntries">
					<xsl:value-of select="$newline"/>
					<xsl:apply-templates select="d:metametadata/d:catalogentry"/>
					<!--see CATALOGENTRY template-->
				</xsl:element>
				<xsl:value-of select="$newline"/>
			</xsl:if>
			...
			<xsl:element name="dateInfo">
				<xsl:attribute name="created"><xsl:choose><xsl:when test="d:metametadata/d:contribute/d:role/d:langstring[contains(.,'Creator')]"><xsl:value-of select="d:metametadata/d:contribute/d:date/d:datetime"/></xsl:when><xsl:otherwise><xsl:text>Need a record creation date</xsl:text></xsl:otherwise></xsl:choose></xsl:attribute>
				<xsl:attribute name="accessioned"><xsl:choose><xsl:when test="string-length(d:metametadata/d:catalogentry/d:accession)>0"><xsl:value-of select="d:metametadata/d:catalogentry/d:accession"/></xsl:when><xsl:otherwise><xsl:text>ERROR:Need a record accession date</xsl:text></xsl:otherwise></xsl:choose></xsl:attribute>
			</xsl:element>
			<xsl:value-of select="$newline"/>
			<!--statusOf-->
			<xsl:element name="statusOf">
				<xsl:attribute name="status"><xsl:choose><xsl:when test="string-length(d:metametadata/d:catalogentry/d:accession)>0"><xsl:value-of select="'Accessioned'"/></xsl:when><xsl:otherwise><xsl:text>Need a record status</xsl:text></xsl:otherwise></xsl:choose></xsl:attribute>
			</xsl:element>
			<xsl:value-of select="$newline"/>
			<!--metaMetadata language-->
			<xsl:apply-templates select="d:metametadata/d:language"/>
			...
			<xsl:element name="termsOfUse">
				...
				<xsl:if test="string-length(d:metametadata/d:metadataTermsOfUse/@URI)>0">
					<xsl:attribute name="URI"><xsl:value-of select="d:metametadata/d:metadataTermsOfUse/@URI"/></xsl:attribute>
				</xsl:if>
			...
			<xsl:if test="string-length($allmroles)>0">
				<xsl:element name="contributors">
					<xsl:value-of select="$newline"/>
					<xsl:apply-templates select="d:metametadata/d:contribute"/>
					<!--see CONTRIBUTE template-->
				</xsl:element>
				<xsl:value-of select="$newline"/>
			</xsl:if>
			...
			<xsl:element name="online">
				...
				<xsl:if test="string-length($tlocation)>0">
					<xsl:element name="mirrorURLs">
						...
						<xsl:apply-templates select="d:technical/d:location[position()>1]"/>
						<!--MIRROR-URL template-->
						<xsl:value-of select="$newline"/>
					</xsl:element>
					<xsl:value-of select="$newline"/>
				...
				<xsl:if test="string-length($allformat)>0">
					<xsl:element name="mediums">
						<xsl:value-of select="$newline"/>
						<xsl:apply-templates select="d:technical/d:format"/>
						<!--see FORMAT template-->
					</xsl:element>
					<xsl:value-of select="$newline"/>
				</xsl:if>
				...
				<xsl:element name="requirements">
					<xsl:value-of select="$newline"/>
					<xsl:apply-templates select="d:technical/d:requirements"/>
					<!--see REQUIREMENTS template-->
				</xsl:element>
				<xsl:value-of select="$newline"/>
		...
		<xsl:element name="educational">
			...
			<xsl:element name="audiences">
				<xsl:value-of select="$newline"/>
				<xsl:apply-templates select="d:educational/d:learningcontext"/>
				<!--see LEARNINGCONTEXT temp.-->
			</xsl:element>
			<xsl:value-of select="$newline"/>
			<!--end audiences above-->
			<!--resourceTypes below-->
			...
			<xsl:element name="resourceTypes">
				<xsl:value-of select="$newline"/>
				<xsl:apply-templates select="d:educational/d:learningresourcetype"/>
				<!--see RESOURCETYPE temp.-->
			</xsl:element>
			<xsl:value-of select="$newline"/>
			...
			<xsl:if test="string-length($allscistd)>0 or string-length($allgeogstd)>0">
				<xsl:element name="contentStandards">
					...
					<xsl:if test="string-length($allscistd)>0">
						<xsl:apply-templates select="d:educational/d:scistd"/>
						<!--see SCIENCE STD template-->
					</xsl:if>
					<!--contentStandard from the DLESE-IMS field of geogstd-->
					<xsl:if test="string-length($allgeogstd)>0">
						<xsl:apply-templates select="d:educational/d:geogstd"/>
						<!-- see GEOGRAPHY STD template-->
					</xsl:if>
				</xsl:element>
				<xsl:value-of select="$newline"/>
			</xsl:if>
			...
			<xsl:if test="string-length(d:educational/d:description/d:langstring)>0">
				<xsl:apply-templates select="d:educational/d:description"/>
				<!--see DESCRIPTION template-->
				<xsl:value-of select="$newline"/>
			</xsl:if>
		</xsl:element>
		...
		<xsl:element name="rights">
			...
			<xsl:value-of select="$newline"/>
			<!--description-->
			<xsl:apply-templates select="d:rights/d:description"/>
			<!--see DESCRIPTION template-->
			<xsl:value-of select="$newline"/>
		</xsl:element>
		<xsl:value-of select="$newline"/>
		...
		<xsl:if test="string-length($allkind)>0">
			<xsl:element name="relations">
				<xsl:value-of select="$newline"/>
				<xsl:apply-templates select="d:relation/d:kind"/>
				<!--see KIND template-->
			</xsl:element>
			<xsl:value-of select="$newline"/>
		</xsl:if>
			...
			<xsl:element name="geospatialCoverages">
				...
				<xsl:element name="geospatialCoverage">
					...
					<xsl:element name="projection">
						<xsl:attribute name="type"><xsl:text>DLESE:Unknown</xsl:text></xsl:attribute>
					</xsl:element>
					<xsl:value-of select="$newline"/>
					<!--coordinateSystem -->
					<xsl:element name="coordinateSystem">
						<xsl:attribute name="type"><xsl:text>DLESE:Geographic latitude and longitude</xsl:text></xsl:attribute>
					</xsl:element>
					<xsl:value-of select="$newline"/>
					<!--boundBox -->
		...
		<xsl:if test="string-length(d:entry/d:langstring)>0">
			<xsl:if test="string-length(d:catalogue)>0">
				...
				<xsl:element name="catalog">
					<xsl:attribute name="entry"><xsl:value-of select="d:entry/d:langstring"/></xsl:attribute>
		...
		<xsl:if test="string-length(d:role/d:langstring)>0">
			<xsl:choose>
				<xsl:when test="string-length(d:centity/d:extension/d:lastname)>0">
					...
					<xsl:apply-templates select="d:centity"/>
					<!--see CENTITY template-->
				</xsl:when>
				...
				<xsl:choose>
					...
					<xsl:when test="d:extension/d:place_event_name/d:langstring[contains(.,'PT:')]">
						...
						<xsl:element name="longLats">
							<xsl:element name="longLat">
								<xsl:attribute name="longitude"><xsl:value-of select="d:extension/d:locations/d:box/d:west"/></xsl:attribute>
								<xsl:attribute name="latitude"><xsl:value-of select="d:extension/d:locations/d:box/d:north"/></xsl:attribute>
							</xsl:element>
						</xsl:element>
					</xsl:when>
					<!-- an east-west line -->
					<xsl:when test="d:extension/d:place_event_name/d:langstring[contains(.,'LNEW:')]">
						...
						<xsl:element name="longLats">
							...
							<xsl:element name="longLat">
								<xsl:attribute name="longitude"><xsl:value-of select="d:extension/d:locations/d:box/d:east"/></xsl:attribute>
								<xsl:attribute name="latitude"><xsl:value-of select="d:extension/d:locations/d:box/d:north"/></xsl:attribute>
							</xsl:element>
						</xsl:element>
					</xsl:when>
					...
					<xsl:when test="d:extension/d:place_event_name/d:langstring[contains(.,'LNNS:')]">
						...
						<xsl:element name="longLats">
							...
							<xsl:element name="longLat">
								<xsl:attribute name="longitude"><xsl:value-of select="d:extension/d:locations/d:box/d:west"/></xsl:attribute>
								<xsl:attribute name="latitude"><xsl:value-of select="d:extension/d:locations/d:box/d:south"/></xsl:attribute>
							</xsl:element>
						</xsl:element>
					</xsl:when>
					<!-- a bounding box -->
	...
	<xsl:template match="d:centity">
		...
		<xsl:element name="contributor">
			<xsl:attribute name="role"><xsl:value-of select="../d:role/d:langstring"/></xsl:attribute>
			<xsl:if test="string-length(../d:date/d:datetime)>0">
				<xsl:attribute name="date"><xsl:value-of select="../d:date/d:datetime"/></xsl:attribute>
			</xsl:if>
			<xsl:value-of select="$newline"/>
				...
				<xsl:element name="timeInfo">
					...
					<xsl:element name="timeAD">
						...
						<xsl:element name="begin">
							...
							<xsl:attribute name="date"><xsl:choose><xsl:when test=".=9999"><xsl:text>Present</xsl:text></xsl:when><xsl:otherwise><xsl:value-of select="."/></xsl:otherwise></xsl:choose></xsl:attribute>
							<xsl:if test="string-length(../d:time)>0">
								<xsl:attribute name="time"><xsl:value-of select="../d:time"/></xsl:attribute>
							</xsl:if>
						</xsl:element>
						<xsl:value-of select="$newline"/>
						<!--end ADN begin date-->
						...
						<xsl:element name="end">
							<!--determine if the value 9999 is present and convert it to the word 'Present'-->
							<xsl:attribute name="date"><xsl:choose><xsl:when test="../../d:endtime/d:datecoverage=9999"><xsl:text>Present</xsl:text></xsl:when><xsl:otherwise><xsl:value-of select="../../d:endtime/d:datecoverage"/></xsl:otherwise></xsl:choose></xsl:attribute>
							<xsl:if test="string-length(../../d:endtime/d:time)>0">
								<xsl:attribute name="time"><xsl:value-of select="../../d:endtime/d:time"/></xsl:attribute>
							</xsl:if>
						</xsl:element>
						<xsl:value-of select="$newline"/>
						<!--end ADN end date-->
				...
				<xsl:choose>
					<xsl:when test="string-length(../d:resource/d:extension/d:location)>0">
						<xsl:element name="urlEntry">
							<xsl:attribute name="kind"><xsl:apply-templates select="d:langstring" mode="kind"/><!--see LANGSTRING/KIND--></xsl:attribute>
							<xsl:attribute name="url"><xsl:value-of select="string(../d:resource/d:extension/d:location)"/></xsl:attribute>
						</xsl:element>
						<xsl:value-of select="$newline"/>
					</xsl:when>
					<xsl:when test="string-length(../d:resource/d:extension/d:catalogentry/d:entry/d:langstring)>0">
						<xsl:element name="idEntry">
							...
							<!--  -->
								<!--for those records that do contain DLESE in relation-->
								<!--assumption DWEL, SERC-EET, SERC-STARTINGPOINT and SERC-NAGT can never make a relation to a DLESE record intentionally. This code assumes all relation to DLESE are unintentional-->
								<xsl:if test="contains(../d:resource/d:extension/d:catalogentry/d:entry/d:langstring, 'DLESE')">
									<xsl:choose>
										<xsl:when test="contains(//d:metametadata/d:catalogentry/d:entry/d:langstring, 'DWEL')">
											<xsl:value-of select="concat('DWEL', substring-after(../d:resource/d:extension/d:catalogentry/d:entry/d:langstring, 'DLESE'))"/>
										</xsl:when>
										<xsl:when test="contains(//d:metametadata/d:catalogentry/d:entry/d:langstring, 'EET')">
											<xsl:value-of select="concat('SERC-EET', substring-after(../d:resource/d:extension/d:catalogentry/d:entry/d:langstring, 'DLESE'))"/>										</xsl:when>
										<xsl:when test="contains(//d:metametadata/d:catalogentry/d:entry/d:langstring,'NAGT')">
											<xsl:value-of select="concat('SERC-NAGT', substring-after(../d:resource/d:extension/d:catalogentry/d:entry/d:langstring, 'DLESE'))"/>										</xsl:when>
										<xsl:when test="contains(//d:metametadata/d:catalogentry/d:entry/d:langstring,'STARTINGPOINT')">
											<xsl:value-of select="concat('SERC-STARTINGPOINT', substring-after(../d:resource/d:extension/d:catalogentry/d:entry/d:langstring, 'DLESE'))"/>
										</xsl:when>
										<xsl:otherwise>
								...
								<!--for those records that do not contain DLESE in relation-->			
								<xsl:if test="not(contains(../d:resource/d:extension/d:catalogentry/d:entry/d:langstring, 'DLESE'))">
									<xsl:value-of select="string(../d:resource/d:extension/d:catalogentry/d:entry/d:langstring)"/>
								</xsl:if>
<!--  -->
<!--								<xsl:value-of select="string(../d:resource/d:extension/d:catalogentry/d:entry/d:langstring)"/>--></xsl:attribute>
						</xsl:element>
						<xsl:value-of select="$newline"/>
					</xsl:when>
				</xsl:choose>
	...
	<xsl:template match="d:min_vertical">
		...
		<xsl:element name="vertMin">
			<xsl:attribute name="units"><xsl:text>meters (m)</xsl:text></xsl:attribute>
			<xsl:value-of select="."/>
		</xsl:element>
		<xsl:value-of select="$newline"/>
		<!--ADN vertMax-->
