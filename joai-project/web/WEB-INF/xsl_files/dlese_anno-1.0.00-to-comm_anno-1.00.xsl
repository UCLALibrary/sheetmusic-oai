...
<!--reading ADN records in webservices-->					
	...
	<!--was working in mar 2011 but not in may 2011 so get explicit with namespaces and express them all
		<xsl:value-of select="document(concat($DDSWSID, $id))//adn:metaMetadata/adn:catalogEntries/adn:catalog/@entry" />
-->
		<xsl:value-of select="document(concat($DDSWSID, $id))//dds:DDSWebService/dds:GetRecord/dds:record/dds:metadata/adn:itemRecord/adn:metaMetadata/adn:catalogEntries/adn:catalog/@entry" />
...
-->
				<xsl:value-of select="document(concat($DDSWSID, $id))//dds:DDSWebService/dds:GetRecord/dds:record/dds:metadata/adn:itemRecord/adn:technical/adn:online/adn:primaryURL"/>
			</xsl:when>
			<xsl:otherwise>
...
<!--do nothing because there is no data to complete a title field and title is optional in comm_anno-->
			...
			<xsl:otherwise> 
				<xsl:element name="title" namespace="http://ns.nsdl.org/ncs/comm_anno">
					<xsl:value-of select="concat('Annotation for ', document(concat($DDSWSID, $id))//dds:DDSWebService/dds:GetRecord/dds:record/dds:metadata/adn:itemRecord/adn:general/adn:title )"/>
