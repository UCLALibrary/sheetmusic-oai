...
public final class SuggestResourceServlet extends HttpServlet {
	...
	public void init(ServletConfig conf)
		...
		super.init(conf);
		System.out.println(getDateStamp() + " SuggestResourceServlet starting");
		String initErrorMsg = "";
		...
		try {
			...
			schemaHelper = new SchemaHelper(schemaUrl, "itemRecord");
		} catch (Throwable e) {
		...
		prtln("schemaHelper instantianted");
 		...
 		File suggestionTemplate = new File(getAbsolutePath("WEB-INF/data/resourceTemplate.xml"));
		if (!suggestionTemplate.exists())
			throw new ServletException ("resourceTemplate not found at " + suggestionTemplate.toString());
 
		SuggestResourceHelper serviceHelper = new SuggestResourceHelper(suggestionTemplate, schemaHelper);
		prtln("instantiated SuggestResourceHelper");
		context.setAttribute("SuggestResourceHelper", serviceHelper); 

		// searchService is used by the DupSimUrlChecker and other classes requiring Search Web Services
		String searchServiceUrl = (String) context.getInitParameter("searchServiceUrl-resource");
		...
		String putServiceUrl = (String) context.getInitParameter("putServiceUrl-resource");
		...
		String putCollection = (String) context.getInitParameter("putCollection-resource");
		...
		serviceHelper.setDcsStatus(
			(String) context.getInitParameter("dcsStatus-resource"));
			
		serviceHelper.setDcsStatusNote(
			(String) context.getInitParameter("dcsStatusNote-resource"));
		
		String emailFrom = (String) context.getInitParameter("emailFrom-resource");
		serviceHelper.setEmailFrom(emailFrom);
			
		String [] emailTo = SuggestUtils.commaDelimitedToArray (
			(String) context.getInitParameter("emailTo-resource"));
		serviceHelper.setEmailTo(emailTo);
			
		String msg = "SuggestResourceServlet configuration";
		...
		// Set all debugging:
/*  	SuggestResourceAction.setDebug(debug);
		SuggestResourceHelper.setDebug(debug);
		SuggestResourceForm.setDebug(debug);
		
		ResourceRecord.setDebug(debug);
		SuggestUtils.setDebug(debug);
		UrlValidator.setDebug (debug); */
		
		System.out.println(getDateStamp() + " SuggestResourceServlet initialized.");
	}
	 ...
	 */
	public void destroy() {
		System.out.println(getDateStamp() + " SuggestResourceServlet stopped");
	}
	...
	private final void prtlnErr(String s) {
		System.err.println(getDateStamp() + " SuggestResourceServlet: " + s);
	}
	...
	private final void prtln(String s) {
		if (debug) {
			System.out.println(getDateStamp() + " SuggestResourceServlet: " + s);
		}
	}
