 ...
 *
 *
 * @version    $Id: Dom4jUtils.java,v 1.21 2011/07/12 04:40:07 ostwald Exp $
 */
public class Dom4jUtils {
	...
	/**
	 *  Load an XML file into a dom4j Document using the given character encoding. If error, return null. No
	 ...
	 */
	public static Document getXmlDocument(URL url, int timoutMs, String userAgent)
		 throws DocumentException, MalformedURLException, IOException, URLConnectionTimedOutException {

		InputStream inputStream = TimedURLConnection.getInputStream(url, timoutMs, userAgent);
		// No validation...
		SAXReader reader = new SAXReader(false);
	 ...
	 * @exception  URLConnectionTimedOutException  If no response before reaching timoutMs
	 */
	public static Document getXmlDocument(String url, int timoutMs, String userAgent)
		 throws DocumentException, MalformedURLException, IOException, URLConnectionTimedOutException {
	...
	/**
	 *  Load XML into a dom4j Document, specifying a max time to wait for resopnse and other arguments. Supports
	 *  gzip transfer of the response. No validation is performed.
	 *
	 * @param  url                                 A URL to an XML document
	 ...
	 * @param  characterEncoding                   Character encoding to use, for example 'UTF-8', or null for
	 *      default
	 * @param  timeoutMs                           Number of milliseconds to wait before timming out, use 0 for
	 *      infinity
	 * @param  localizeXml                         True to localize the XML Document
	 * @param  userAgent                           The http User-Agent header sent with the request, or null to
	 *      use default
	 * @return                                     An XML Document containing the dom4j DOM, or null if unable to
	 *      process the file.
	 ...
	 * @exception  URLConnectionTimedOutException  If no response before reaching timoutMs
	 */
	public static Document getXmlDocument(String url, String postData, String characterEncoding, int timeoutMs, boolean localizeXml, String userAgent)
		 throws DocumentException, MalformedURLException, IOException, URLConnectionTimedOutException {

		if (characterEncoding == null && postData == null && !localizeXml)
			return getXmlDocument(url, timeoutMs, userAgent);

		Document document = getXmlDocument(TimedURLConnection.importURL(url, postData, characterEncoding, timeoutMs, userAgent));
		if (localizeXml)
			document = localizeXml(document);
		return document;
	}
	 ...
	 * @exception  URLConnectionTimedOutException  If no response before reaching timoutMs
	 */
	public static Document getXmlDocumentPostData(URL url, String postData, int timoutMs, String userAgent)
		 throws DocumentException, MalformedURLException, IOException, URLConnectionTimedOutException {
		InputStream inputStream = TimedURLConnection.getInputStream(url, postData, timoutMs, userAgent);
		// No validation...
		SAXReader reader = new SAXReader(false);
		Document document = reader.read(inputStream);
		return document;
	 ...
	 * @exception  TransformerConfigurationException  If trnasform error
	...
	public static String getNameSpaceInfo(Document doc, String rootElementName) {
		...
		if (!m.find()) {
			// pattern will not be found if this is simple xml doc with no namespace or xs attributes in root
	...
	/**
	 *  Writes {@link org.dom4j.Document} to File using UTF-8 encoding.
	 *
	...
	/**
	 *  Write XML Document to disk using specified OutputFormat using UTF-8 encoding.
	 *
	 * @param  doc            Documument to write
	 * @param  out            File to write to
