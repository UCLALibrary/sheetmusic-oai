 ...
 *  education level, and type. Uses groups files to define the data elements and vocabularies. Lazy-loads the
 *  groups files and caches them once loaded. Create a new NSDLDCNormalizer to release the cache and refresh
 *  the vocabularies.<p>
 *
 *  Groups files have been saved to http://ns.nsdl.org/ncs/ddsws/1-1/groupsNormal/
 *
...
public class NSDLDCNormalizer {
	 ...
	 *      record
	 */
	public Document normalizeNsdlDcRecord(Document nsdlDcRecordDoc, String metadataHandle, List resourceHandles, String nsdlPartnerId, List requiredXpaths) throws NSDLDCNormalizerXpathException {
		...
		try {
			...
			// Insert the NSDL metadata handle:
			...
			for (int i = 0; i < _xpathsToModify.size(); i++) {
				Map foundValues = new HashMap(); // Track values that have been created to avoid duplicating
					...
					for (int j = 0; j < fromXpathsList.size(); j++) {
						...
						for (int k = 0; k < existingDCElms.size(); k++) {
							...
							for (int m = 0; m < outlineNodes.size(); m++) {
								...
								List newValueOutlineNodes = outlineNode.selectNodes(".[@type='group']|..[@type='group']|../..[@type='group']"); // Go one and two levels up, for nested vocabs like EdLevel
			...
			String comment = "The National Science Digital Library (NSDL) has normalized this metadata record for use across its systems and services.";
			...
			newDoc.getRootElement().addComment(comment);
