...
public final class XMLConversionService {
	...
	public StringBuffer getConvertedXml(String fromFormat,
		...
		if (fromFormat.equals(toFormat)) {
			...
			else if (originalXMLFile != null && originalXMLFile.canRead()) {
				try {
					if (filter)
						return stripXmlDeclaration(new BufferedReader(
		...
		if(luceneDoc != null)
			origLastModified = luceneDoc.getContentLastModifiedTime();
		else
			origLastModified = originalXMLFile.lastModified();		
		...
		if (cachedFile.isFile() && cachedFile.lastModified() > origLastModified &&
