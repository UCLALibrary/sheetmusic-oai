...
public class SchemaUtils {
	...
	public static SchemaHelper getRegisteredSchemaHelper (String schemaName) throws Exception {
		SchemaRegistry sr = new SchemaRegistry();
		
		SchemaRegistry.Schema schema = (SchemaRegistry.Schema)sr.getSchema(schemaName);
		if (schema == null) {
			throw new Exception ("Registered schema not found for \"" + schemaName + "\"");
		}
		
		String path = schema.path;
		String rootElementName = schema.rootElementName;
		return getSchemaHelper (path, rootElementName);
	}
	
	public static SchemaHelper getSchemaHelper (String schemaUri) {
		return getSchemaHelper (schemaUri, null);
	}
		
	public static SchemaHelper getSchemaHelper (String schemaUri, String rootElementName) {
		SchemaHelper sh = null;
		try {
			if (schemaUri.indexOf ("http:") == 0) {
				URL	schemaUrl = new URL (schemaUri);
				sh = new SchemaHelper(schemaUrl, rootElementName);
				// sh = new SchemaHelper(schemaUrl);
			}
			else {
				prtln ("schemaUri (file): " + schemaUri);
				sh = new SchemaHelper (new File (schemaUri), rootElementName);
			}
			...
			prtln ("getSchemaHelper error: " + e.getMessage());
			return null;
		}
	}
	...
	/**
	 *  Returns listing of all derivedDataTypes for framework represented by schemaHelper.
	 *
	...
	/**
	 *  Returns listing of a schemaNodeMap for provided schemaHelper
	 *
	...
	public static void showComboUnionFields(SchemaHelper schemaHelper) {
		...
		prtln("xsd:Combo union typeDefs");
		List comboUnions = new ArrayList();
		...
		prtln (comboUnions.size() + " combo union typeDefs found");
		for (Iterator i=comboUnions.iterator(); i.hasNext(); ) {
			GlobalDef def = (GlobalDef)i.next();
			prtln(NL + "name: " + def.getName());
