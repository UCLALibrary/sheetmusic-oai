...
import org.dlese.dpc.services.dds.FacetsBean;
 ...
 * @version    $Id: DDSServicesAction.java,v 1.63 2011/01/26 00:49:46 jweather Exp $
...
public class DDSServicesAction extends Action {
	...
	protected ActionForward doSearch(
		...
		if (index != null && !isEmptySearch) {
			...
			resultDocs = index.searchDocs(booleanQuery, dateFilter, sortObject, docReaderAttributes);
			...
			// Handle Faceting. Accepts Solr-style arguments facet=true, facet.field={fieldName}:
			...
			if(facet != null && facet.equals("true")) {
				...
				if (facetFields != null) {
					FacetsBean facetsBean = new FacetsBean (booleanQuery, facetFields, rm);
