...
public final class DCSServicesAction extends Action {
	...
	protected ActionForward doPutRecord(
		...
		/* If the record EXISTS, 
				...
				repositoryServices.saveEditedRecord 
		   else
			   if status and statusNotes not supplied,
		   			use defaults
		...
		*/
		...
		try {
			...
			if (!recordExists) {
				String dcsStatus = (statusParam != null ? statusParam : StatusFlags.IMPORTED_STATUS);
				String dcsStatusNote = (statusNoteParam != null ? statusNoteParam : 
																  "imported via DCS PutRecord Service");
				...
				try {
					...
					if (statusParam != null || statusNoteParam != null) {
						if (statusParam == null) {
							// reuse the current status
							statusParam = repositoryService.getDcsDataRecord(idParam).getStatus();
						}
						StatusEntry statusEntry = new StatusEntry (statusParam, statusNoteParam, Constants.UNKNOWN_EDITOR);
