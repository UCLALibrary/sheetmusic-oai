 ...
 * @version    $Id: ScheduledHarvestManager.java,v 1.25 2011/04/05 01:08:24 jweather Exp $
...
public final class ScheduledHarvestManager {
	 ...
	 * @param  userAgent            The http User-Agent header sent with the request, or null to use system
	 *      default
	 */
	public ScheduledHarvestManager(SimpleDataStore dataStore, File initialHarvestDir, SimpleLuceneIndex harvestLogIndex, int timeOutMilliseconds, String userAgent) {
		this.ds = dataStore;
	...
	public Hashtable getScheduledHarvests() {
	    System.out.println(" In ScheduledHarvestManager SimpleDataStore - " + ds.get(Keys.SCHEDULED_HARVESTS));
	...
	private void performHarvest(Long shUid, boolean harvestAll, boolean harvestAllIfNoDeletedRecord, boolean override) {
		...
		synchronized (this) {
			...
			IndexingHarvestMsgHandler handler =
				...
				sh.getUid().toString(),
				50, sh.getHarvestDir());
			harvester = new Harvester(handler, timeOutMilliseconds, userAgent);
	 ...
	 *      successfully.
	 */
	private Date doHarvest(String repositoryName,
	                       Harvester harvester,
	                       String baseURL,
