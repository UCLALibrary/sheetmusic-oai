...
public final class FileIndexingService {
	...
	public void stopIndexing() {
		...
		try {
			primaryIndex.stopIndexing(); // Stop the indexer
	...
	public boolean addDirectory(
		...
		if (primaryIndex != null && documentWriterConfigAttributes.get("index") == null)
			documentWriterConfigAttributes.put("index", primaryIndex);
	...
	public void indexFile(File fileToIndex, FileIndexingPlugin plugin)
		...
		synchronized (_indexingSync) {
			...
			try {
				...
				recordDataService = (RecordDataService) primaryIndex.getAttribute("recordDataService");
				...
				List existingDocs = primaryIndex.listDocs("docsource", ab);
				...
				if (newDocData != null && newDocData.getDoc() != null) {
					...
					removeDocsFromIndex(primaryIndex, docsToRemove);
					// Add the new document:
					primaryIndex.update("docsource", ab, newDocData.getDoc(), SimpleLuceneIndex.BLOCK);
				}
				else
	...
	private void synchIndexWithFiles(boolean reindexAll, File directory, FileIndexingObserver observer) {
		synchronized (_indexingSync) {
			...
			try {
				result = synchDirectory(directory, reindexAll, primaryIndex);
			...
			if (result == INDEXING_ERROR) {
				message = "Errors occured while indexing directory '" + directory.getName() + "'. Some or all items did not " +
					"get indexed or re-indexed. Total number of items in index: " + primaryIndex.getNumDocs();
				addIndexingMessage(message);
			}
				...
				message = "Completed indexing directory '" + directory.getName() +
					"' successfully, however one or more items was indexed with errors. Total time "
					 + elapsedTime + ". Total number of items in index: " + primaryIndex.getNumDocs();
				addIndexingMessage(message);
			}
				...
				message = "Completed indexing directory '" + directory.getName() +
					"' successfully. Total time " + elapsedTime + ". Total number of items in index: " + primaryIndex.getNumDocs();
				addIndexingMessage(message);
			}
			// Notify the observer
			try {
		...
		synchronized (_indexingSync) {
			...
			synchronized (sourceDirs) {
				...
				addIndexingMessage("Beginning indexing of " + sourceDirs.size() +
					" directories. Total number of items in index: " + primaryIndex.getNumDocs());
			}
			...
			try {
				for (int i = 0; i < dirs.length; i++) {
					result = synchDirectory(dirs[i].srcDir, reindexAll, primaryIndex);
					Thread.yield();
				}
			} catch (FileIndexingServiceException e) {
				addIndexingMessage(e.getMessage());
			...
			if (result == INDEXING_ERROR) {
				...
				synchronized (sourceDirs) {
					message = "Completed indexing " + sourceDirs.size() +
						" directories successfully, however indexing was requested to stop. Some or all directories may not have been indexed. "
						 + "Total time " + elapsedTime + ". Total number of items in index: " + primaryIndex.getNumDocs();
				}
				addIndexingMessage(message);
			}
			else {
				synchronized (sourceDirs) {
					message = "Completed indexing " + sourceDirs.size() +
						" directories successfully. Total time " + elapsedTime + ". Total number of items in index: " + primaryIndex.getNumDocs();
				}
				addIndexingMessage(message);
			}
			// Notify the observer
	...
	private final void removeDeletedDirs() {
		try {
			...
			List indexedDirs = primaryIndex.getTerms("docdir_remove");
			String indexedDir;
			File dirFile;
			if (indexedDirs != null) {
	...
	public final void removeDocs(
		...
		synchronized (_indexingSync) {
			...
			if (saveDeletes) {
				List docsToDelete = primaryIndex.listDocs(field, terms);
				Document d;
				Document dd;
				ArrayList deletedDocs = new ArrayList(docsToDelete.size());
				if (docWriter == null) {
					//prtln("removeDocs1(): docWriter is null. Skipping update... ");
				}
				else {
					for (int i = 0; i < docsToDelete.size(); i++) {
						d = (Document) docsToDelete.get(i);
						try {
							dd = docWriter.getDeletedDoc(d);
							if (dd != null) {
								deletedDocs.add(dd);
							}
						} catch (Throwable e) {
				...
				//prtln("removeDocs1(): removing " + deletedDocs.size() + " docs to status deleted");
				primaryIndex.update(field, terms, (Document[]) deletedDocs.toArray(new Document[]{}), true);
			}
			else {
				//prtln("removeDocs2(): removing docs completely from the index");
				primaryIndex.removeDocs(field, terms);
			}
			numIndexingQue--;
		}
	}
	...
	public int reindexDocs(String field, String term, boolean reindexAll) {
		List updateDocs = primaryIndex.listDocs(field, term);
	...
	public int reindexDocs(String field, String[] terms, boolean reindexAll) {
		List updateDocs = primaryIndex.listDocs(field, terms);
		new UpdateThread((Document[]) updateDocs.toArray(new Document[]{}), reindexAll).start();
		return updateDocs.size();
	}
	...
	public int reindexDocs(String query, boolean reindexAll) {
		ResultDocList results = primaryIndex.searchDocs(query);
	...
	private class UpdateThread extends Thread {
		...
		public void run() {
			try {
				doIndexObjects(updateObjects, sourceDir, reindexAll, primaryIndex);
			} catch (FileIndexingServiceException e) {
				addIndexingMessage(e.getMessage());
	 ...
	 * @param  index                             The index to store the entries.
	 * @param  docsToUpdateObj                   NOT YET DOCUMENTED
	 * @return                                   The final indexing status
	 * @exception  FileIndexingServiceException  If error.
	 */
	private int doIndexObjects(
		...
		synchronized (_indexingSync) {
			...
			try {
				...
				if (dirDocs != null) {
					// Create a DocWriter if it wasn't already in the loop:
					if (myDocWriter == null && object_type == FILE_ARRAY) {
						myDocWriter = getNewFileIndexingServiceWriter(srcDir.getAbsolutePath());
						myDocWriter.setValidationEnabled(validateFiles);
					}
					for (int i = 0; i < dirDocs.size() && !stopIndexing; i++) {
						Thread.yield();
						rDoc = (Document) dirDocs.get(i);
						...
						if (!filesHash.containsKey(ab)) {
							...
							if (saveDeletes) {
								if (deletedStr == null || !deletedStr.equals("true")) {
									try {
										...
										/*
											Note: We may not want to index deletion failures as errors.
											Might be better just to output the error to logs for developer debugging.
										*/
										if (false && indexErrors) {
											try {
												errorOccured = true;
												//prtln("Indexing an error for deleted doc: " + e);
												//e.printStackTrace();
												FileIndexingServiceException fe =
													new FileIndexingServiceException("Problem creating deleted record entry. " + e.getMessage());
												ErrorFileIndexingWriter errDoc =
													new ErrorFileIndexingWriter(fe);
												fileToRemove = new File(ab);
												newDocData = errDoc.create(fileToRemove, null, null, indexingSessionAttributes);
												if (newDocData != null && newDocData.getDoc() != null) {
													addDocs.add(newDocData.getDoc());
												}
												addRemoveFields(newDocData, removeItemsHashMap);
											} catch (Throwable ee) {
												prtlnErr("Error creating error Doc for deleted record: " + ee);
												//ee.printStackTrace();
											}
										}
										else {
							...
							// If not tracking deletes, remove altogether
							else {
								removeFilesByPath.add(ab);
								num_remove++;
								//prtln("synchDirectory(): removing a deleted file completely");
							}
						}
					}
				}
