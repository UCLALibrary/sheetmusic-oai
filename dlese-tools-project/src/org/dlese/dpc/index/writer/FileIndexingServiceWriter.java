...
public abstract class FileIndexingServiceWriter implements DocWriter {
	 ...
	 * @param  previousRecordDoc  An existing Document that currently resides in the index for the given
	 ...
	 *  <code>}</code>
	 *
	 * @param  newDoc             The new {@link org.apache.lucene.document.Document} that is being created for
	 *      this resource
	 * @param  previousRecordDoc  An existing {@link org.apache.lucene.document.Document} that currently resides
	 *      in the index for the given resource, or null if none was previously present
	 * @param  sourceFile         The sourceFile that is being indexed
	 * @exception  Exception      This method should throw and Exception with appropriate error message if an
	 *      error occurs.
	 */
	...
	public String getFileContent() throws IOException {
		if (fileContent == null) {
			...
			/*
			// Some debugging:
			prtln("getFileContent() for source'" + source + "' doesExist:" + source.exists() + "' is: " + fileContent);
			prtln("previousRecordDoc isNull:" + (previousRecordDoc == null));
			if(previousRecordDoc != null)
				prtln("previousRecordDoc filecontent: " + previousRecordDoc.get("filecontent"));
			*/
		}
		return fileContent;
	}
	 ...
	 *
	 * @return    The configuration attributes, or empty Map if none configured
	 */
	...
	/**
	 *  Sets the Lucene Document that this Writer is building. Normally, this would be created internally, but
	 *  this method may be used to set a specific Document to work with such as the case when indexing a
	 *  multirecord.
	 *
	 * @param  luceneDoc  The new luceneDoc value
	 */
	public void setLuceneDoc(Document luceneDoc) {
		this.luceneDoc = luceneDoc;
	}


	/**
	 *  Gets the previous Document that currently resides in the index for the given resource, or null if none
	 ...
	 *
	 * @param  previousRecordDoc  An existing FileIndexingService Document that currently resides in the index
	 *      for the given file
	...
	public synchronized FileIndexingServiceData create(File sourceFile,
		 ...
		 */
			...
			if (plugin != null) {
				//prtln("Using passed-in plugin: " + plugin.getClass().getName());
			...
			// Else if we have a pre-configured FileIndexingPlugin, use it...
				...
				//prtln("Using existing plugin: " + fileIndexingPlugin.getClass().getName());
				fileIndexingPlugin.addFields(sourceFile, doc, existingLuceneDoc, getDocType(), getDocGroup());
			}
			else {
				//prtln("plugins are null!");
			}
