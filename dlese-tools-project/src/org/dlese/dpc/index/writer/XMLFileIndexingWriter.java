...
import org.dlese.dpc.repository.indexing.SessionIndexingPlugin;
...
public abstract class XMLFileIndexingWriter extends FileIndexingServiceWriter {
	...
	protected void addCustomFields(Document newDoc, Document existingDoc, File sourceFile) throws Exception {
		...
		if (dleseCollectionDocReader != null) {
			...
			collectionXmlIndexer.indexXpathFields(newDoc);
		}
		...
		String oaimodtime = getOaiModtime(existingDoc);
		if (oaimodtime != null)
			...
			if (ids != null && ids.length > 0) {
				...
				for (int i = 0; i < ids.length; i++) {
					...
					if (i == 0) {
						...
						if (!getSourceFile().getName().equals("temp_dds_repository_put_record_file.xml")) {
	...
	private boolean doResourceDeDuplication(Document newDoc, Document existingDoc, File sourceFile) throws Exception {
		...
		if (getPrimaryId().equals("temp_dds_repository_put_record_file"))
			return false;
	...
	private void indexIndividualDupDoc(String id, String coll, String[] sessionIds, File sourceFile2, boolean fetchFromPrimaryIndex, boolean indexDupIndexDoc, boolean indexMultirecordData) throws Exception {
		...
		if (indexDupIndexDoc) {
			...
			try {
				newData = xmlWriter.create(sourceFile2, myExistingDoc, plugin, null);
				doc = newData.getDoc();
			} catch (Throwable e) {
				e.printStackTrace();
			...
			if (doc != null) {
				...
				dupItemsIndex.update("idvalue", id, doc, true);
			}
		}
		...
		if (indexMultirecordData) {
			...
			HashMap newConfig = (HashMap) getConfigAttributes().clone();
			newConfig.put("collection", coll);
			xmlWriter.setLuceneDoc(getLuceneDoc()); // Pass in my Document to add data to for multirecord
				...
				for (int i = 0; i < relatedDocs.size(); i++) {
					...
					if (myRelationTypes != null) {
						for (int j = 0; j < myRelationTypes.size(); j++) {
							...
							String collectionKey = xmlDocReader.getCollectionKey();
	...
	/**
	 *  Index crawled content from the ContentCache web service, if configured.
	 *
	 ...
	 */
	public final String getOaiModtime(Document existingDoc) {
		try {
			String newXMLVersion = getFileContent();
			String prevXMLVersion = null;
			if (existingDoc != null)
				prevXMLVersion = existingDoc.get("filecontent");
			...
			if (newXMLVersion != null && prevXMLVersion != null && prevXMLVersion.equals(newXMLVersion)) {
				String oaimodtime = existingDoc.get("oaimodtime");
				if (oaimodtime != null) {
					return oaimodtime;
				}
			}
