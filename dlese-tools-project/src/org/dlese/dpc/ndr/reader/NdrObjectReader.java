...
public class NdrObjectReader {
	...
	public static NdrObjectReader getConcreteReader (NdrObjectReader reader) throws Exception {
		NDRConstants.NDRObjectType objectType = reader.getObjectType();
		if (objectType == NDRConstants.NDRObjectType.AGENT)
			return new AgentReader (reader.getDocument());
		if (objectType == NDRConstants.NDRObjectType.AGGREGATOR)
			return new AggregatorReader (reader.getDocument());
		if (objectType == NDRConstants.NDRObjectType.METADATA)
			return new MetadataReader (reader.getDocument());	
		if (objectType == NDRConstants.NDRObjectType.METADATAPROVIDER)
			return new AgentReader (reader.getDocument());	
		return reader;
	}
	...
	public Element getDataStream(String format, String version) {
		...
		if (this.getFormats().contains(formatSpec)) {
			...
			prtlnErr("\t getDataStream(): object (" + getHandle() + " does NOT contain format: " + format);
		}
		return stream;
	}
