...
public class SimpleNdrRequest {
	...
	/**
	* Construct the URI for this request, which is the ndrApiBaseUrl plus the request verb, and
	* in some cases, an object handle
	*/
	protected String makePath () throws Exception {
		String ndrApiBaseUrl = NDRConstants.getNdrApiBaseUrl();
		if (ndrApiBaseUrl == null || ndrApiBaseUrl.trim().length() == 0)
			throw new Exception ("NdrRequest cannot determin ndrApiBaseUrl - check ndr config");
		if (verb == null || verb.trim().length() == 0)
			throw new Exception("attempting to submit request without specifying verb");
		String path = ndrApiBaseUrl + "/" + this.verb;
