...
public class NdrUtils {
	...
	public static void setup(String ndrServer) {
		...
		NDRConstants.init(ndrServer, null, null, null);
	}
	...
	public static void setup(File propsFile) {
		...
		try {
			...
			userAgent = props.getProp("ndr.user.agent", "");
			NdrUtils.initProp();
		} catch (Throwable t) {
			prtln("initProp error: " + t.getMessage());
		}

		NDRConstants.init(ndrServer, ncsAgentHandle, keyPath, userAgent);
	}

	/**
	 *  Intialize NDR Constants to enable interaction with NDR.
	 *
	 * @param  ndrServer       the server to which to send requests
	 * @param  ncsAgentHandle  agentHandle
	 * @param  keyPath         path to keyword file
	 * @param  userAget        userAgent for NDR API requests
	 */
	public static void setup(String ndrServer, String ncsAgentHandle, String keyPath, String userAgent) {
		try {
			NdrUtils.initProp();
		} catch (Throwable t) {
			prtln("initProp error: " + t.getMessage());
	...
	public static Document getNDRObjectDoc(URL handleUrl) throws Exception {
		...
		String userAgent = NDRConstants.getUserAgent();
		String content = null;
		try {
			content = TimedURLConnection.importURL(handleUrl.toString(), null, "utf-8", millis, userAgent);
