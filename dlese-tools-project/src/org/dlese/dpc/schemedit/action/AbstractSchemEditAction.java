...
public abstract class AbstractSchemEditAction extends DCSAction {
	...
	public ActionForward execute(
		...
		try {
			...
			if (command.equals("vocabTypeAhead")) {
				return handleVocabTypeAhead (mapping, form, request, response);
			}
	...
	protected ActionForward handleVocabTypeAhead (ActionMapping mapping,
	                                            ActionForm form,
	                                            HttpServletRequest request,
	                                            HttpServletResponse response) {
													
		prtln ("handleVocabTypeAhead");
		SchemEditForm sef = (SchemEditForm) form;
		...
		try {
			if (value == null || value.trim().length() == 0)
				throw new Exception ("value param not found");
			if (path == null || path.trim().length() == 0)
				throw new Exception ("path param not found");		
			List allValues = sef.getLegalEnumerationValuesOf (path);
			for (Iterator i=allValues.iterator();i.hasNext();) {
				String term = (String)i.next();
				if (term.toLowerCase().indexOf(value.toLowerCase()) != -1)
					filteredValues.add (term);
/* 				if (filteredValues.size() > 30)
					break; */
			}
			
			// Make json - should be incorporated in above loop!!
			prtln (filteredValues.size() + " terms matched");
			Element jsonXml = DocumentHelper.createElement("vocabTypeAhead");
			for (Iterator i=filteredValues.iterator();i.hasNext();) {
				String term = (String)i.next();
				// prtln ("- " + term);
				jsonXml.addElement("value").setText(term);
			}

			org.json.JSONObject json = org.json.XML.toJSONObject(jsonXml.asXML());
			sef.setAsyncJason (json.toString());
			
			// sef.setAsyncJason ("{\'values\':[\'fee\',\'fii\']}");
		} catch (Exception e) {
			prtlnErr ("handleVocabTypeAhead error: " + e.getMessage());
		}
