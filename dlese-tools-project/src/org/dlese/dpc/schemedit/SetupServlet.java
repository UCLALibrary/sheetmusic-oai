...
import org.dlese.dpc.schemedit.threadedservices.NLDRProperties;
...
public final class SetupServlet extends HttpServlet {
	...
	public void init(ServletConfig config)
		...
		try {
			standardsServiceSetup();
		} catch (Exception e) {
			prtlnErr ("StandardsServiceSetup ERROR: " + e.getMessage());
		}
		
		String nldrPropsPath = (String) getServletContext().getInitParameter("nldrProperties");
		if (nldrPropsPath != null && nldrPropsPath.trim().length() > 0) {
			File nldrProps = new File (nldrPropsPath.trim());
			if (!nldrProps.exists())
				throw new ServletException ("configured nldrProperties file does not exist at: " + nldrPropsPath);
			try {
				OsmRecordExporter.setNLDRProperties (new NLDRProperties (nldrPropsPath));
			} catch (Exception e) {
				throw new ServletException ("could not instantiate NLDRProperties for " + nldrPropsPath);
			}
			prtln ("props set for OsmRecordExporter");
		}
	 ...
	 *  to by the "standardsServiceConfig" init param, which is used to
	 ...
	 * @exception  ServletException  Description of the Exception
	 */
	void standardsServiceSetup() throws ServletException {
		prtln("\nstandardsServiceSetup() ...");
		String errMsg;
		
		// try to find standardsServiceConfig file
		File configFile = null;
		
		String configParam =
			(String) getServletContext().getInitParameter("standardsServiceConfig");
		...
		File standardsLibrary = null;
		String libPath = getAbsolutePath((String) getServletContext().getInitParameter("asnStandardsLibrary"));
		if (libPath != null && libPath.trim().length() > 0) {
			standardsLibrary = new File (libPath);
			if (!standardsLibrary.exists()) {
				prtln ("library does not exist at " + libPath + ".  creating ...");
				if (!standardsLibrary.mkdir())
					throw new ServletException ("could not create libPath at " + libPath);
			}
				
			StandardsRegistry.setLibraryDir (standardsLibrary);
			prtln ("initialized libraryDir in StandardsRegistry (" + libPath + ")");
		}

		
		// initialize asnCatalog/AsnClientToolKit
		String asn2acsrIdMappings = getAbsolutePath("WEB-INF/data/ACSR-to-ASN-Mappings");
		if (! (new File (asn2acsrIdMappings).exists()))
			throw new ServletException ("asn2acsrIdMappings directory does not exist at " + asn2acsrIdMappings);
		org.dlese.dpc.serviceclients.asn.AsnClientToolkit.setAsnIdMapperData (asn2acsrIdMappings);
		AsnCatalog.setMappingsDirectory(asn2acsrIdMappings);
		...
		if (suggestionServiceManager != null && suggestionServiceManager.hasConfig("adn")) {
