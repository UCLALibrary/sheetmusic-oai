...
public class DleseEditorRenderer extends EditorRenderer {
	...
	public Element getMultiBoxInput(String xpath) {
		...
		if (rhelper.hasVocabLayout(xpath)) {
			prtln("\t has LAYOUT");
			Element layoutMultiBox = DocumentHelper.createElement("vl__vocabLayoutMultiBox")
			...
			Element mdvMultiBox = DocumentHelper.createElement(getMultiBoxTag())
					.addAttribute("elementPath", "enumerationValuesOf(" + XPathUtils.getSiblingXPath(xpath) + ")");
			...
			prtln("\t hasFieldInfo");
			Element fieldInfoMultiBox = DocumentHelper.createElement("st__fieldInfoMultiBox")
		...
		if (sm != null && normalizedXPath.equals(sm.getXpath())) {
			...
			try {
				...
				Element standardsTag = DocumentHelper.createElement("std__" + sm.getRendererTag())
						.addAttribute("elementPath", "enumerationValuesOf(" + XPathUtils.getSiblingXPath(xpath) + ")")
	...
	public Element getSelectInput(String xpath) {
		...
		if (rhelper.hasVocabLayout(normalizedXPath)) {
			Element layoutSingleSelect = DocumentHelper.createElement("vl__vocabLayoutSingleSelect")
					.addAttribute("elementPath", "valueOf(" + xpath + ")");
			return layoutSingleSelect;
		}
		// does this field have a controlled vocab? - see if it is a typeAhead path
		if (this.typeDef.isSimpleType() && ((SimpleType)this.typeDef).isEnumerationType()) {

			List typeAheadPaths = this.rhelper.getFramework().getSchemaPathMap().getTypeAheadXPaths();
			if (typeAheadPaths.contains (XPathUtils.normalizeXPath(xpath))) {
				int maxLen = rhelper.getMaxLen(xpath);
				Element typeAhead = DocumentHelper.createElement("html__text")
					.addAttribute ("name", "sef")
					.addAttribute ("property", "valueOf(" + xpath + ")")
					.addAttribute("size", String.valueOf(maxLen))
					.addAttribute("styleId", "${id}")
					.addAttribute("styleClass", "vocab-type-ahead");
				return typeAhead;
			}
		}
