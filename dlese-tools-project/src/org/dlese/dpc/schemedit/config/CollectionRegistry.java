 ...
 * @author     ostwald <p>
 *
 *      $Id: CollectionRegistry.java,v 1.32 2010/09/28 19:23:59 ostwald Exp $
 * @version    $Id: CollectionRegistry.java,v 1.32 2010/09/28 19:23:59 ostwald Exp $
 */
	...
	*/
	...
	public void initializeIDGenerators(SimpleLuceneIndex index) {
		...
		for (Iterator i = cols.iterator(); i.hasNext(); ) {
			/* as of 9/2020, the "collection" field contains 2 entries per collection,
			   one with a "0" prefix, and one without. we ignore entries without a leading "0"
			*/
			String collectionTerm = (String)i.next();
			if (collectionTerm.charAt(0) != '0')
				continue;
			
			// the collection term is "0"+collectionKey
			String collectionKey = collectionTerm.substring(1);
