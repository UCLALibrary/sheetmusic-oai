...
public class SchemaPathMap {
	...
	*/
	...
	/**
	 *  Returns list of SchemaPath instances having specified valueType (e.g.,'type_ahead' or 'url', etc).
	 *
	...
	private List typeAheadPaths = null;

	public List getTypeAheadXPaths() {
		if (this.typeAheadPaths == null) {
			ArrayList paths = new ArrayList();
			for (Iterator i = nameMap.values().iterator(); i.hasNext(); ) {
				SchemaPath schemaPath = (SchemaPath) i.next();
				if (schemaPath.valueType != null && schemaPath.valueType.equals("type_ahead"))
					paths.add(schemaPath.xpath);
			}
			this.typeAheadPaths = paths;
		}
		return this.typeAheadPaths;
	}
