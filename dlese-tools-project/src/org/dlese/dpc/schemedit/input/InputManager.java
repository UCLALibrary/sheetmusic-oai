...
public class InputManager {
	...
	private InputField createInputField(String paramName,
		...
		value = SchemEditUtils.stripScripts (value);
	...
	private void addField(String paramName, InputField inputField) {
		...
		// we don't want to call single-value fields with vocabLayouts multiValueFields
		...
		else if (isMultiValueField(inputField)) {
	 ...
	 *  MultiValue (DO WE NEED THIS ANY MORE, OR WAS IT A TEMPORARY KLUDGE)??<p>
	 
	 NOTE: this method contains a giant KLUDGE to work around the problem that we don't
	 know if a field governed by a groups file accepts mulitple values or a single value!!
	 CONSIDER modifying groups file format to specify this....
	 *
	...
	private boolean isMultiValueField(InputField field) {
		...
		String normxpath = field.getNormalizedXPath();
		String xmlFormat = this.framework.getXmlFormat();
		boolean hasVocabLayout = false;
		try {
			hasVocabLayout = this.framework.getVocabLayouts().hasVocabLayout(normxpath);
		} catch (Throwable t) {}
		if (hasVocabLayout) {
			prtln("hasVocabLayout: " + normxpath);
		}
		
		boolean isMultiSelect = schemaHelper.isMultiSelect(field.getSchemaNode());
		...
		if (hasVocabLayout) {
			// KLUDGE for contentBundle, which has an attribute and accepts a single value
			if (xmlFormat.equals("math_path") && normxpath.equals("/record/educational/contentBundle"))
				return false;
			else
				return true;
		}
		if (this.framework.getStandardsManager() != null &&
				this.framework.getStandardsManager().getXpath().equals(normxpath)) {
