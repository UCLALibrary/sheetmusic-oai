...
public class ReferenceResolver {
	...
	public static void main(String[] args)
		...
		String in = "&#169;"; // isDefined (using char codePoint AND codePointValue);
		in = "&#x2093;";
		in = "&#x2088;";
		prtln("in:  " + in);
		ReferenceResolver rr = new ReferenceResolver();
 		ResolverResults results = rr.resolve(in);
		prtln("out: " + results.content);
	...
	static String toUnicode(String s)
		...
		try {
			...
			// in 1.4.2 isDefined requires a character (but 1.5 does not....)
 			if (!Character.isDefined(codePoint)) {
				prtln ("-> NOT defined ... " + codePointChar);
