...
/* 		} catch (IOException ioe) {
	...
	public static void main(String[] args) throws Exception {
		...
		String format = "math_path";  // dcs_data news_opps adn framework_config
...
/*		MetaDataFramework framework = tester.getFramework();
		...
		tester.mathPathTester();

	}

	void mathPathTester() {

		// String xpath = "/record/general/subjects/mathSubject";
		String xpath = "/record/educational/contentBundle";
		MetaDataFramework framework = this.getFramework();
		SchemaHelper sh = framework.getSchemaHelper();
		...
		try {
			hasVocabLayout = framework.getVocabLayouts().hasVocabLayout(xpath);
		} catch (Throwable t) {}
		if (hasVocabLayout) {
			prtln("hasVocabLayout: " + xpath);
		}

/* 		boolean isMultiSelect = sh.isMultiSelect(schemaNode);
		prtln ("isMultiSelect: " + isMultiSelect);

		boolean isRepeating = sh.isRepeatingElement(schemaNode);
		prtln ("isRepeating: " + isRepeating);

		boolean isSingleton = sh.isSingleton(schemaNode);
		prtln ("isSingleton: " + isSingleton); */

		// prtln (typeDef.toString());
/* 		boolean isEnumerationType = sh.isEnumerationType(typeDef);
		prtln ("isEnumerationType: " + isEnumerationType); */

		if (hasVocabLayout) {
			if (xmlFormat.equals("math_path") && xpath.equals("/record/educational/contentBundle"))
				prtln ("SINGLE");
			else
				prtln ("MULTI");
		}
