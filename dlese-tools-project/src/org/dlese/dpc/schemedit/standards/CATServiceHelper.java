...
import org.dlese.dpc.schemedit.standards.asn.CommCoreServiceHelper;
...
public abstract class CATServiceHelper implements SuggestionServiceHelper {
	...
	public static CATServiceHelper getInstance(SchemEditForm sef)
		...
		prtln ("getInstance() xmlFormat: " + sef.getXmlFormat());
		...
		while (enumeration.hasMoreElements()) {
			...
			if (param.toLowerCase().startsWith("catserviceplugin")) {
				String[] splits = ((String)servletContext.getInitParameter(param)).split("\\|");
				
				String xmlFormat = splits[0].trim();
				if (xmlFormat.equals(sef.getXmlFormat())) {
					pluginClassName = splits[1].trim();
					break;
				}
			}
		}
		...
		try {
			CATHelperPlugin plugin = null;
			if (pluginClassName != null) {
				Class pluginClass = Class.forName(pluginClassName);
				plugin = (CATHelperPlugin) pluginClass.newInstance();
			}
			if (plugin == null) {
			...
			// No longer support comm_core as a home-grown standards
/* 			else if (sef.getXmlFormat().equals("comm_core")) {
				helper = new CommCoreServiceHelper(sef, plugin);
			} */
			
			// Another one-off that was impelemented as a demo but not used
			...
			// check context to see if the CAT Suggestions are disabled
			String catServiceIsActive  =
				(String) servletContext.getAttribute("catServiceIsActive");
			prtln ("catServiceIsActive: " + catServiceIsActive);
			helper.setServiceIsActive("true".equals (catServiceIsActive) && plugin != null);
			
			return helper;
		} catch (Throwable e) {
			String errMsg = "Error loading SuggestionServiceHelper class '" + sef.getXmlFormat() + "'. " + e;
			e.printStackTrace();
	...
	protected List getSelectedStandards(Document doc) {
		...
		if (nodes == null) {
			...
			// prtln ("\nInstanceDoc:" + org.dlese.dpc.xml.Dom4jUtils.prettyPrint(doc));
			return new ArrayList();
		}
	...
	public void updateStandardsDisplay(String displayContent) throws Exception {
		...
		if (standardsTree == null) {
			...
			prtlnErr("updateStandardsDisplay: standardsTree not found, bailing ..");
			return;
		}
