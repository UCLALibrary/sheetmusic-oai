...
/**
 *  Reader for ncs_collect metadata records, which are used to manage NSDL collections in the NDR.
 *
...
public class NCSCollectReader {
	...
	/**
	* Returns the values for viewContexts contained in this record, or an empty list
	* if none are present.
	*/
	public List getViewContexts () {
		return this.getValues("/record/collection/viewContexts/viewContext");
	}
	
	/**
	* Returns true if specified viewContext value is present in this NCSCollect record.
	*/
	public boolean hasViewContext (String viewContext) {
		return (this.getViewContexts().contains(viewContext));
	}
	...
	public static void main(String[] args) throws Exception {
		...
		if (readFromFile) {
			String path = "c:/tmp/ncs-collect-tester.xml";
			Document ncs_collect = Dom4jUtils.getXmlDocument(new File(path)); 
/* 			File dir = new File("C:/Documents and Settings/ostwald/devel/dcs-instance-data/local-ndr/records/ncs_collect/1256687231238");
		...
		prtln ("ViewContexts");
		for (Iterator i=reader.getViewContexts().iterator();i.hasNext();)
			prtln ("  " + (String)i.next());
		
		String vc = "NSDL.org";
		prtln ("has " + vc + " viewContext? " + reader.hasViewContext(vc));
	}


	/**  debugging utility for serviceDescriptions */
	...
	/**
	 *  Utility to get a node at the specified xpath.
	 *
	 * @param  xpath  path to desired node
	 ...
	 *
	 * @param  xpath  xpath at which values are located
	 ...
	 *
	 * @param  xpath  xpath to location of new value
	 ...
	 *  exists.
	 *
	 * @param  xpath  path to location of value
