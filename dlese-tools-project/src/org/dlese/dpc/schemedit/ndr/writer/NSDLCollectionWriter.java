...
public class NSDLCollectionWriter extends MetadataWriter {
	...
	private String updateAggregator()
		...
		if (collectionAggregator == null) {
			...
			// only authorize FEED_EATER_AGENT if we are in NSDL environment!
			if (NDRConstants.getNcsAgent().equals(NDRConstants.NCS_PROD_AGENT))
	...
	private String updateMetadataProvider() throws Exception {
		...
		// as of 9/22/2010, a viewContext value of "NSDL.org" is required for public OAIVisibility
		if (this.finalAndValid && ncsCollectReader.hasViewContext("NSDL.org")) {
