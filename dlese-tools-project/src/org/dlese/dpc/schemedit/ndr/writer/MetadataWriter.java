...
public class MetadataWriter {
	...
	protected void init(String recordXml) throws Exception {
		...
		this.docReader = RepositoryUtils.getXMLDocReader(recId, rm);
		if (docReader == null) {
	...
	protected NdrRequest getWriteRequest(String resourceUrl,
		...
		/* 4-2011 - write uniqueID each time to ensure NDR is in sync with NCS
		- addresses problem where we migrate oai collections to ncs and the uniqueID
		  prop is out of sync with NCS Id
		*/
		ndrRequest.addCommand("property", "uniqueID", this.recId);
	...
	protected NdrRequest addMetadataRequest(NdrRequest ndrRequest,
		...
		// ndrRequest.addCommand("property", "uniqueID", recId); // 4/2011 now we write uniqueID each time
