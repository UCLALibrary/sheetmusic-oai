...
public class RepositoryService {
	...
	/**
	 *  Save an edited record to disk and update the index accordingly.<p>
	 NOTE: Record must exist in index (otherwise use {@link #saveNewRecord}.<p>
	 The metadata Documentn is validated and the dcsDataRecord updated with validation
	 and lastTouch and lastEditor information (the status note and status flags are unchanged).
	 
	 *
	 * @param  recId          id of metadata record to be saved
	 * @param  doc            metadata record as dom4j.Document
	 * @param  user           the sessionUser who is saving the document
	 * @exception  Exception  if record is not found in index or if unable to successfully update index
	 */
	public void saveEditedRecord(String recId, Document doc, User user) throws Exception {

		XMLDocReader docReader = RepositoryUtils.getXMLDocReader(recId, this.getRepositoryManager());
		if (docReader == null)
	 ...
	 * @param  username       username of record author
	 ...
	 */
		public void saveNewRecord(String recId,
	                          ...
	                          String username)
		 throws Exception {
			 saveNewRecord (recId, recordXml, collection, username, null, null);
		 }
	...
	public void saveNewRecord(String recId,
		...
		// update status
		dcsDataRecord.setLastTouchDate(getDateString());
		String myStatus = (status != null ? status : StatusFlags.NEW_STATUS);
		String myStatusNote = (statusNote != null ? statusNote : "Record Created");
		String myUsername = (username != null ? username : Constants.UNKNOWN_EDITOR);
		dcsDataRecord.updateStatus(myStatus, myStatusNote, myUsername);
	 ...
	 */
	 ...
	 public String moveRecord(String recId, String collection)
		...
		XMLDocReader docReader = RepositoryUtils.getXMLDocReader(recId, rm);
		String xmlFormat = docReader.getNativeFormat();
	...
	public void deleteCollection(String collection) throws Exception {
		...
		prtln ("rm.delete collection: " + collection);
		getRepositoryManager().deleteCollection(collection);
	}
	...
	public XMLDocReader copyRecord(String originalId, User user)
		...
		XMLDocReader docReader = RepositoryUtils.getXMLDocReader(originalId, rm);

		String collection = docReader.getCollection();
		String originalXml = docReader.getXml();
		String xmlFormat = docReader.getNativeFormat();
		File originalFile = RepositoryUtils.getFileFromIndex(originalId, rm);
		...
		return RepositoryUtils.getXMLDocReader(newId, rm);
	}
	...
	public String copyMoveRecord(String srcRecordId, String destCollection)
		...
		// extract info from the record to be moved
		XMLDocReader docReader = RepositoryUtils.getXMLDocReader(srcRecordId, rm);
		String xmlFormat = docReader.getNativeFormat();
	...
	public XMLDocReader getXMLDocReader(String id) throws Exception {
		...
		return RepositoryUtils.getXMLDocReader(id, rm);
	}
