...
public class LoadMetadataOPML implements org.xml.sax.ContentHandler {
	...
	public void load() {
		...
		try {
			...
			if ( servletContext != null ) {
				...
				prtln("Loaded vocab '" + contextAttributeName + "' to the servlet context.");
			}
			else {
				prtln("Did not load vocab '" + contextAttributeName + "' to the servlet context (servletContext is null).");
				returnVocab = vocab;
			}
		}
		catch ( Exception e ) {
	...
	private final void prtln( String s ) {
		if ( debug ) {
			System.out.println( getDateStamp() + " LoadMetadataOPML: " + s );
		}
	}
