...
public class OAISetsXMLConfigManager {
	 ...
	 * @exception  Exception   If error
	 */
	public static void putOAISetSpecDefinition(File setsConfigFile, SetDefinitionsForm sb) throws Exception {
		...
		String[] matchingFieldValues = sb.getMatchingFieldValues();

		//prtln("putOAISetSpecDefinition(): setSpec:" + setSpec);
		...
		try {
			...
			// ----- Define the set using *either* matchingFieldValues *or* virtualSearchField: -----

			// Define the set using the matchingFieldValues:
			if (matchingFieldValues != null && matchingFieldValues.length > 0) {
				
				// Add the matchingFieldValues element:
				Element matchingFieldValuesElm = set.addElement("matchingFieldValues");
				
				for(int i = 0; i < matchingFieldValues.length; i++) {
					// Each field/value String delimited by colon:
					String fieldValuePair[] = matchingFieldValues[i].split(":");
					if(fieldValuePair.length != 2)
						throw new Exception("Invalid matchingFieldValues String  '" + matchingFieldValues[i] + "'");
					
					Element matchingFieldValueElm = matchingFieldValuesElm.addElement("matchingFieldValue");
					matchingFieldValueElm.addAttribute("field",fieldValuePair[0]);
					matchingFieldValueElm.addAttribute("value",fieldValuePair[1]);
				}
			}
			// Otherwise, define the set using a virtualSearchField:
			else {
			...
			writer.write(document); */
			Dom4jUtils.writePrettyDocToFile(document, setsConfigFile);
