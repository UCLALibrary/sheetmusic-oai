...
public class RepositoryManager {
	...
	public int init(boolean indexCollectionRecords) {
		...
		xmlFileIndexingWriterFactory = new XMLFileIndexingWriterFactory(recordDataService, getIndex(), xmlIndexerFieldsConfig);
		...
		if (removeDocs)
			fileIndexingService = new FileIndexingService(primaryIndex, 0,
				repositoryDataDir.getAbsolutePath() + "/file_indexing_service_data", maxFilesToIndex);
		else
			fileIndexingService = new FileIndexingService(primaryIndex, 0, true, "id",
				repositoryDataDir.getAbsolutePath() + "/file_indexing_service_data", maxFilesToIndex);
		...
		dupItemsIndex = new SimpleLuceneIndex(primaryIndex.getIndexLocation() + "/dup_items_index", primaryIndex.getAnalyzer(), isReadOnly);
	...
	/**
	 *  Optimize the repository indexes for faster searching.
	 *
	 * @exception  UpdateNotPermittedException  If read-only
	 */
	public void optimizeIndexes() throws UpdateNotPermittedException {
		if (isReadOnly)
			throw new UpdateNotPermittedException("Can not optimize index.");
		try {
			if (primaryIndex != null)
			...
			throw new UpdateNotPermittedException("Can not optimize index: " + we);
		}
	}
	...
	private void createPrimaryIndex() throws Exception {
		...
		SimpleLuceneIndex newIndex = new SimpleLuceneIndex(indexLocation, perFieldAnalyzer, isReadOnly);
	 ...
	 */
	public RepositoryManager(File repositoryConfigDir, String repositoryDataDir, int updateFrequency, int maxFilesToIndex, boolean removeDocs, boolean readOnly) {
	...
	public ResultDocList getOdlQueryResults(
		...
		ResultDocList results = primaryIndex.searchDocs(fullQuery, dateFilter);
		int num = 0;
		if (results != null)
			num = results.size();
	...
	private String doPutRecord(String recordXml,
		...
		synchronized (publicUpdateApiLock) {
			...
			File tempFile = new File(System.getProperty("java.io.tmpdir"), "temp_dds_repository_put_record_file.xml");
			...
			try {
				...
				if (newRecordId.equals("temp_dds_repository_put_record_file")) {
					if (id == null || id.trim().length() == 0)
				...
				if (indexRelations) {
					...
					if (existingRelationsIds != null) {
						for (int i = 0; i < existingRelationsIds.size(); i++) {
							if (!idsToReindex.contains(existingRelationsIds.get(i)))
								idsToReindex.add(existingRelationsIds.get(i));
						}
					}
	 ...
	 * @exception  RecordUpdateException        If record ID does not exist or unable to reindex the record for
	 *      any reason
	 * @exception  UpdateNotPermittedException  If read-only
	 */
	...
	public int getNumRecordsInSet(String setSpec) {
		...
		if (query != null) {
			...
			ResultDocList results = primaryIndex.searchDocs(bq);
			if (results == null)
				return 0;
			else
				return results.size();
