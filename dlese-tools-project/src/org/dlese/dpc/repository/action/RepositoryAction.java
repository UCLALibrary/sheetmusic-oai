 ...
 * @version    $Id: RepositoryAction.java,v 1.41 2011/04/05 01:08:24 jweather Exp $
 */
public final class RepositoryAction extends Action {
	...
	public ActionForward execute(
		...
		try {
			...
			if (verb == null) {
				rf.addOaiError(OAICodes.BAD_VERB, "The request must specify a verb");
			...
			if(verbs.length > 1) {
				rf.addOaiError(OAICodes.BAD_VERB, "Only one verb argument must be present in the request, but " + verbs.length + " were found");
				return (mapping.findForward("oaipmh.error"));				
			}
			...
			if (isMySetDisabled) {
				...
				if (!statusDeleted && (content == null || content.trim().length() == 0)) {
					if (!rm.isIdInRepository(idStripped)) {
						...
						String availFormats = "";
						try {
							availFormats = " Available formats are: " + Arrays.toString(rm.getAvailableFormatsList(idStripped).toArray());
						} catch (Throwable t) {}
						rf.addOaiError(OAICodes.CANNOT_DISSEMINATE_FORMAT,
							"The format '" + format +
							"' is not available for identifier '" +
							identifier + "'." + availFormats);
					}
				}
