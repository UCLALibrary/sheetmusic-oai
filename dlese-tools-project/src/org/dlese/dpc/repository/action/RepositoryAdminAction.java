...
public final class RepositoryAdminAction extends Action {
	...
	public ActionForward execute(
		...
		MetadataVocab vocab = rm.getMetadataVocab();
		...
		// Set up the bean:
		setBeanWithData(raf, req, isIndexedByIndexingManager);
		...
		try {
			...
			if (command != null) {
				...
				// Load a saved repository:
				if (command.equals("loadSavedRepository")) {
					DDSServlet ddsServlet = (DDSServlet) servletContext.getAttribute("ddsServlet");

					//String repositoryDirName = req.getParameter("repositoryDirName");
					try {
						ddsServlet.commitBackgroundIndex();

						// Re-load the bean to capture new data from the repository manager
						setBeanWithData(raf, req, isIndexedByIndexingManager);
					} catch (Throwable t) {
						errors.add("error", new ActionError("generic.error", "Could not load saved repository: " + t));
						saveErrors(req, errors);
						return mapping.findForward("display.repository.settings");
					}
					errors.add("message", new ActionError("generic.message", "The repository has been loaded."));
					saveErrors(req, errors);
					return mapping.findForward("display.repository.settings");
				}
			...
			if (req.getParameter("currentSetSpec") != null) {
				...
				if (vocab != null &&
					...
					DDSServlet.setCollectionsVocabDisplay(rm);
				}
			}
	...
	private ActionForward doAddSetDefinition(ActionErrors errors,
		...
		try {
			rm.putOAISetSpecDefinition(setDefinitionsForm);
		} catch (Throwable t) {
	...
	private void setBeanWithData(RepositoryAdminForm raf, HttpServletRequest req, boolean isIndexedByIndexingManager) {
		try {
			ServletContext servletContext = getServlet().getServletContext();

			// Get the latest instance of RepositoryManager, which may have been updated after first load:
			RepositoryManager rm =
				(RepositoryManager) servletContext.getAttribute("repositoryManager");
			...
			prtlnErr("Error setBeanWithData(): " + t);
